use burn::{tensor::{backend::Backend, Tensor}, module::Module, config::Config, nn::{GroupNormConfig, LinearConfig}};

use crate::Model;

use super::{linear::{LinearNetwork, LinearNetworkConfig}, Activation, find_nth_factor, Normalize};
...
impl<B: Backend> Model<B> for LinearAutoEncoder<B> {
    ...
    fn from_config(config: Self::Config) -> Self {
        ...
        for i in 0..config.half_depth {
            ...
            let layer_output_size = (config.input_size as f64 - step * (i + 1) as f64).round() as usize;
            
            let norm = config.normalize.then(|| {
                GroupNormConfig::new(find_nth_factor(layer_output_size, config.normalize_group_idx).unwrap_or(layer_output_size), layer_output_size).with_epsilon(config.normalize_eps)
            });
            linears.push((LinearConfig::new(layer_input_size, layer_output_size), norm, config.activation));
        }
        let encoder = Model::from_config(LinearNetworkConfig {
            linears,
        });
        ...
        for i in 0..config.half_depth {
            ...
            let layer_output_size = (config.latent_dim as f64 + step * (i + 1) as f64).round() as usize;
        ...
        let decoder = Model::from_config(LinearNetworkConfig {
            linears,
        });
        Self {
            encoder,
